...
def can_be_created_with_a_hash_of_attributes
  ...
  Movie.new(attributes).tap{|m| m.save}
end
...
def can_update_using_update_method
  ...
  Movie.create(title: "Wat?").tap{|m| m.update(title: "Wat, huh?")}
end

def can_update_multiple_items_at_once
  # Change title of all movies to "A Movie"
  5.times do |i|
    Movie.create(title: "Movie_#{i}", release_date: 2000+i).tap{|m| m.update(title: "A Movie") }
  end
end

def can_destroy_a_single_item
  Movie.create(title: "That One Where the Guy Kicks Another Guy Once").tap{|m| m.destroy}
end
